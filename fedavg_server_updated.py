...
class FedAvgServer:
    ...
    def run(self):
        ...
        if self.trainer.epoch_scores:
          import json
          client_in_total = self.args.dataset_args["client_num_in_total"]
          file_name = f"data_file_epoch_gl{self.args.global_epoch}_le{self.args.local_epoch}_cn{client_in_total}.json"
          full_path = OUT_DIR / self.algo / file_name
          
          with open(full_path, 'w') as file:
            json.dump(self.trainer.epoch_scores, file)
          # print("lal", self.trainer.epoch_scores)

        if self.args.save_log:
            self.logger.save_text(OUT_DIR / self.algo / f"{self.args.dataset}_gl{self.args.global_epoch}_log.html")
        ...
        if self.args.save_fig:
            ...
            plt.legend()
            plt.savefig(
                OUT_DIR / self.algo / f"{self.args.dataset}_ge{self.args.global_epoch}.jpeg", bbox_inches="tight"
            )
        if self.args.save_metrics:
            ...
            pd.DataFrame(np.stack(accuracies, axis=1), columns=labels).to_csv(
                OUT_DIR / self.algo / f"{self.args.dataset}_ge{self.args.global_epoch}_acc_metrics.csv",
                index=False,
            )
